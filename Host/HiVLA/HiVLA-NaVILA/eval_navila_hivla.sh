#!/bin/bash
# NaVILA-HiVLA Pipeline Runner
#
# Usage:
#   bash eval_navila_hivla.sh                  # all splits, full pipeline (1→7)
#   bash eval_navila_hivla.sh train             # train only (steps 1→6)
#   bash eval_navila_hivla.sh val_seen          # val_seen only (steps 2→3)
#   bash eval_navila_hivla.sh val_unseen        # val_unseen only (steps 2→3)
#   bash eval_navila_hivla.sh 2                 # step 2 on train + val_seen + val_unseen
#   bash eval_navila_hivla.sh 3                 # step 3 on train + val_seen + val_unseen
#   bash eval_navila_hivla.sh 7                 # step 7 only
#   bash eval_navila_hivla.sh train 1           # step 1 only
#   bash eval_navila_hivla.sh train 2           # step 2 on train only
#   bash eval_navila_hivla.sh val_seen 2        # step 2 on val_seen only
#
# Steps:
...
CONFIG_PATH="vlnce_baselines/config/r2r_baselines/navila_hivla.yaml"
...
TRAIN_EPISODES=1000         # Steps 1+2: total train episodes (split evenly across chunks)
...
DIR_CMP="${OUT_BASE}/7_comparison"   # Step 7 comparison outputs
# ──────────────────────────────────────────────────────────────

IFS=',' read -ra GPULIST <<< "$GPU_LIST"
CHUNKS=${#GPULIST[@]}

# Internal helper: launch NaVILA evaluation across chunks
_run_eval() {
...
_step1() {
    ...
    _run_eval "${DIR_HTMP}" \
        EVAL.SPLIT train \
        EVAL.EPISODE_COUNT $EPS_PER_CHUNK \
        HIVLA.HTMP_EXTRACTION.ENABLED True \
        HIVLA.HTMP_EXTRACTION.EXTRACT_H_TMP True \
        HIVLA.HTMP_EXTRACTION.SPL_THRESHOLD $SPL_THRESHOLD \
        HIVLA.DATA_EXTRACTION.ENABLED False
    echo "  Head ranking → ${DIR_HTMP}/temporal_head_importance.json"
}

_step2() {
    local SPLIT="${1:-val_seen}"
    local EPS_COUNT=$( [ "$SPLIT" = "train" ] && echo $(( TRAIN_EPISODES / CHUNKS )) || echo -1 )
    echo "=== Step 2: Data Extraction (${SPLIT}, eps/chunk=${EPS_COUNT}) ==="
    mkdir -p "${DIR_DATA}/${SPLIT}"
    _run_eval "${DIR_DATA}/${SPLIT}" \
        EVAL.SPLIT $SPLIT \
        EVAL.EPISODE_COUNT $EPS_COUNT \
        HIVLA.DATA_EXTRACTION.ENABLED True \
        HIVLA.DATA_EXTRACTION.OUTPUT_DIR "${DIR_DATA}/${SPLIT}" \
        HIVLA.DATA_EXTRACTION.TEMPORAL_HEAD_IMPORTANCE_PATH "${DIR_HTMP}/$(basename ${MODEL_PATH})/VLN-CE-v1/train/temporal_head_importance.json" \
        HIVLA.HTMP_EXTRACTION.ENABLED False
...
_step3() {
    ...
    python vlnce_baselines/hivla/3_gt_labeling/waypoint_gt_labeling.py \
...
_step4() {
    ...
    python vlnce_baselines/hivla/4_hnav_selection/hnav_selection.py \
...
_step5() {
    ...
    python vlnce_baselines/hivla/5_grid_search/grid_search.py \
        --entropy_dir "${DIR_DATA}/${SPLIT}" \
        --gt_dir      "${DIR_GT}/${SPLIT}" \
...
_step6() {
    ...
    python vlnce_baselines/hivla/6_figures/generate_figures.py \
...
_step7() {
    ...
    if [ -f "${DIR_GT}/val_seen/episode_diagnostics.jsonl" ]; then
        ...
        echo "  [WARN] val_seen GT not found — run: bash eval_navila_hivla.sh val_seen 3"
    fi
    ...
    if [ -f "${DIR_GT}/val_unseen/episode_diagnostics.jsonl" ]; then
        ...
        echo "  [WARN] val_unseen GT not found — run: bash eval_navila_hivla.sh val_unseen 3"
    fi

    python vlnce_baselines/hivla/7_comparison/compare_from_gt.py \
        $ARGS \
...
if [ -z "$ARG1" ]; then
    # No args → full pipeline across all splits
    echo "=== HiVLA Full Pipeline (train → val_seen → val_unseen → step 7) ==="
    _pipeline_all

elif [[ "$ARG1" =~ ^[0-9]+$ ]]; then
    # Bare step number → run that step across all relevant splits
    case "$ARG1" in
        1) _step1 ;;
        2) _step2 train; _step2 val_seen; _step2 val_unseen ;;
        3) _step3 train; _step3 val_seen; _step3 val_unseen ;;
        4) _step4 train ;;
        5) _step5 train ;;
        6) _step6 ;;
        7) _step7 ;;
        *) echo "Unknown step: $ARG1"; exit 1 ;;
    esac
    ...
    echo ""
    echo "Usage:"
    echo "  bash eval_navila_hivla.sh                  # full pipeline (all splits)"
    echo "  bash eval_navila_hivla.sh train             # train pipeline (steps 1→5)"
    echo "  bash eval_navila_hivla.sh val_seen          # val_seen pipeline (steps 2→3)"
    echo "  bash eval_navila_hivla.sh val_unseen        # val_unseen pipeline (steps 2→3)"
    echo "  bash eval_navila_hivla.sh 7                 # step 7 only"
    echo "  bash eval_navila_hivla.sh train 1           # step 1 only"
    echo "  bash eval_navila_hivla.sh train 2           # step 2 on train"
    echo "  bash eval_navila_hivla.sh val_seen 2        # step 2 on val_seen"
    echo "  bash eval_navila_hivla.sh val_seen 3        # step 3 on val_seen"
    exit 1
fi
