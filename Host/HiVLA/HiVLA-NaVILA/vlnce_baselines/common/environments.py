...
class VLNCEDaggerEnv(habitat.RLEnv):
    ...
    def save_agent_state(self) -> Dict[str, float]:
        pos = self._env.sim.get_agent_state().position
...
class VLNCEWaypointEnvDiscretized(VLNCEWaypointEnv):
    ...
    def reset(self) -> Observations:
        ...
        if self.video_option:
            ...
            self.video_frames = [navigator_video_frame(observations, info, start_pos, start_heading)]

        return observations

    def step(self, action: Union[int, str, Dict[str, Any]], *args, **kwargs) -> Tuple[Observations, Any, bool, dict]:
        observations = None
        ...
        if action != "STOP":
            ...
            if len(plan) == 0:
                agent_state = self._env.sim.get_agent_state()
                observations = self._env.sim.get_observations_at(agent_state.position, agent_state.rotation)
